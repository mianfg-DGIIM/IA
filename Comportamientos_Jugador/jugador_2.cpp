...
Action ComportamientoJugador::think(Sensores sensores) {
	bool visto = false;
	//estado loc_aldeano = 
	//hay_aldeano_delante = true;// (sensores.superficie[2] == 'a');
	//if(hay_aldeano_delante) cout << "======= HAY ALDEANO DELANTE =======\n";
	...
	if ( sensores.nivel == 4 ) {
		...
		*/
		...
		if ( sensores.superficie[2] == 'a' ) {
			...
			// si tenemos un aldeano delante recalculamos
			//hayplan = false;
		} else {
	...
	if (!hayplan || visto) {
		...
		if ( aRecarga && !recargas.empty() ) {
			cout << "VA A RECARGAR"<< endl;
			estado siguiente = loc_recarga;
...
*/
	...
	switch ( mapaResultado[sensores.posF][sensores.posC] ) {
		case 'K':	// guardar bikini
			has_bikini = true;
			break;
		case 'D':	// guardar zapatillas
			has_zapatillas = true;
			break;
		case 'X':	// efectuar recarga una vez que estamos en casilla de recarga
			if ( aRecarga ) {
				// WIP si pasamos por casualidad por la casilla de recarga??
				aRecarga = false;
				hayplan = true;
				plan.clear();
				/*
				...
				*/
				//WIP que dependa de los trayectos medios
	...
	if (hayplan and plan.size()>0) {
		...
		/*if ( sensores.nivel == 4 ) {
			...
			if ( sigAccion == actFORWARD )
				if ( hay_aldeano_delante ) {
					// si vamos a desplazarnos hacia delante y tenemos un aldeano en frente
					// recalcularemos el path para esquivar el aldeano
					hayplan = pathFinding(sensores.nivel, actual, siguiente, plan);
					...
					if ( plan.empty() ) {
						...
						sigAccion = plan.front();
					// si no hay forma de esquivarlo, paramos y recalculamos en el siguiente think
...
bool ComportamientoJugador::pathFinding (int level, const estado &origen, const estado &destino, list<Action> &plan){
	switch (level){
		case 1: //Qcout << "Busqueda en profundad\n";
			      return pathFinding_AStar(origen,destino,plan);
						break;
		case 2: //Qcout << "Busqueda en Anchura\n";
			      return pathFinding_Anchura2(origen,destino,plan);
						break;
		case 3: //Qcout << "Busqueda Costo Uniforme\n";
				  return pathFinding_CostoUniforme2(origen,destino,plan);
						break;
		case 4: //Qcout << "Busqueda para el reto\n";
				  return pathFinding_AStar(origen,destino,plan);
						break;
	}
	//Qcout << "Comportamiento sin implementar\n";
	return false;
}
...
bool ComportamientoJugador::HayObstaculoDelante(estado &st){
  ...
  // calculo cual es la casilla de delante del agente
	...
	//WIP
	// AÑADIDO miramos si en frente tenemos un aldeano
	if ( fil == loc_aldeano_delante.fila and col == loc_aldeano_delante.columna )
		return false;
...
bool ComportamientoJugador::pathFinding_AStar(const estado &origen, const estado &destino, list<Action> &plan) {
	...
	// el mínimo camino desde el origen
	if ( esDestino(actual.st, destino) ) {
		//Qcout << "Cargando el plan\n";
		while ( actual.st != origen ) {
			plan.push_front(viene_de[actual].second);
			actual = viene_de[actual].first;
		}
		//Qcout << "Longitud del plan: " << plan.size() << endl;
		//QPintaPlan(plan);
		// ver el plan en el mapa
		VisualizaPlan(origen, plan);
