...
Action ComportamientoJugador::think(Sensores sensores) {
	...
	if ( sensores.nivel == 4 ) {
		...
		// vemos si tenemos un aldeano delante
		if ( sensores.superficie[2] == 'a' ) {
			loc_aldeano_delante.fila = actual.fila;
			loc_aldeano_delante.columna = actual.columna;
			switch ( sensores.sentido ) {
				case norte:	loc_aldeano_delante.fila--; break;
				case sur:	loc_aldeano_delante.fila++; break;
				case oeste:	loc_aldeano_delante.columna--; break;
				case este:	loc_aldeano_delante.columna++; break;
			}
		} else {
			loc_aldeano_delante.fila = -1;
			loc_aldeano_delante.columna = -1;
		}
		...
		El jugador recargará la batería de acuerdo a los siguientes criterios:
			- si la batería es inferior a B_I
				- si se encuentra cerca de una recarga
		...
		Donde:
			B_I es un valor fijo
			B_S se calcula en función del tamaño del mapa
		...
		*/
		...
		if ( sensores.bateria <= b_i && !hayplan ){
			// si el destino se encuentra cerca, iremos a él
			if ( heuristica(actual, destino) > heuristica(actual, loc_recarga)/K_DESTINO )
				aRecarga = true;
		} else if ( b_i < sensores.bateria && sensores.bateria < b_s
			// añadimos una condición extra: si queda poco tiempo, sólo recargará
			// en el caso de que el nivel de la batería sea crítico (<=B_I)
			/* MEJORA 4. Evitando ciclos en las recargas */
			&& sensores.vida >= TIEMPO_RECARGA_INMEDIATA && !ultimo_recarga ){
	...
	if ( aRecarga && !recargas.empty() ) {
		siguiente = loc_recarga;
		/* MEJORA 5. Ir a por el objetivo si está de camino a la recarga */
		// si en el camino a la recarga se encuentra con un objetivo
		if ( heuristica(actual, destino) < mapaResultado.size()/K_CERCANIA ) {
			siguiente = destino;
			hayplan = false;
		}
	
	/* MEJORA 3. Capturando el bikini y las zapatillas */
	// si no tiene bikini y lo ve, va a por él
	} else if ( !has_bikini && !bikinis.empty() ) {
		siguiente = *bikinis.begin();
		//cout << "He visto el bikini y voy a por él\n";
	} else if ( !has_zapatillas && !zapatillas.empty() ) {
		siguiente = *zapatillas.begin();
	
	} else
		siguiente = destino;

	if (!hayplan || visto) {
		hayplan = pathFinding(sensores.nivel, actual, siguiente, plan);
	}
	...
	switch ( mapaResultado[sensores.posF][sensores.posC] ) {
		...
		case 'X':	// efectuar recarga una vez que estamos en casilla de recarga
			...
			if ( aRecarga ) {
				...
				int cantidad_recarga = -3*sensores.bateria/140 + 325/7;
	...
	if (hayplan and plan.size()>0) {
		...
		if ( sensores.nivel == 4 ) {
			if ( sigAccion == actFORWARD ) {
				/* MEJORA 1. Esquivando aldeanos */
				// si vamos adelante y tenemos un aldeano en frente
				if ( loc_aldeano_delante.fila != -1 ) {
					// recalculamos
					hayplan = pathFinding(sensores.nivel, actual, siguiente, plan);
					if ( hayplan ) {
						sigAccion = plan.front();
...
bool ComportamientoJugador::HayObstaculoDelante(estado &st){
  ...
  // calculo cual es la casilla de delante del agente
	...
	// Compruebo si hay una aldeano delante
	if ( loc_aldeano_delante.fila == fil and loc_aldeano_delante.columna == col )
		return false;
...
int ComportamientoJugador::peso(const char &celda) const {
	if		( celda == 'A' )	return has_bikini ? 10 : 100;
	else if	( celda == 'B' )	return has_zapatillas ? 5 : 50;
	else if	( celda == 'T' )	return 2;
	else if	( celda == 'R' )	return 0;
	else						return 1;
}
	...
	for (int i = 0; i <=3; i++) {
		for (int j=0; j < 2*i+1; j++) {
			if ( mapaResultado[F+j*a_f][C+j*a_c] == '?' ) {
				mapa_actualizado = true;
				// actualizamos el peso medio para '?'

				mapaResultado[F+j*a_f][C+j*a_c]= sensores.terreno[a];

				if ( sensores.terreno[a] == 'X' )
					recargas.insert(estado{F+j*a_f, C+j*a_c, -1});
				else if ( sensores.terreno[a] == 'K' && !has_bikini )
					bikinis.insert(estado{F+j*a_f, C+j*a_c, -1});
				else if ( sensores.terreno[a] == 'D' && !has_zapatillas )
					zapatillas.insert(estado{F+j*a_f, C+j*a_c, -1});
			}
